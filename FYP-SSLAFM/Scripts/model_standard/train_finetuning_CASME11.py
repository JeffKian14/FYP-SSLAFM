...
def train_finetuning():
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomResizedCrop(size=224, scale=(0.92,1.0)),   
        transforms.ToTensor(), 
    ])
    ...
    # --- Store all fold histories for the final comparison plots ---
    all_folds_history = []
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_dataset_full)):
        ...
        class_weights = torch.tensor([124.0/32.0, 124.0/88.0, 124.0/25.0], dtype=torch.float32).to(DEVICE)
        criterion = nn.CrossEntropyLoss(weight=class_weights)
        
        optimizer = optim.Adam([
            {'params': model_s2.parameters(), 'lr': 2e-5}, 
        ...
        for epoch in range(EPOCHS):
            ...
            print(f"Fold {fold+1} | Epoch [{epoch+1}/{EPOCHS}] | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f} | UF1: {UF1:.4f} | UAR: {UAR:.4f} | Val Acc: {epoch_val_acc*100:.2f}%")
            ...
            if UF1 > best_val_uf1:
                ...
                torch.save(model_s2.state_dict(), os.path.join(current_dir, f"stage3_finetuned_backbone_c11_fold{fold+1}.pth"))
                torch.save(classifier.state_dict(), os.path.join(current_dir, f"stage3_best_classifier_c11_fold{fold+1}.pth"))
            else:
                patience_counter += 1
        ...
        fig = make_subplots(rows=2, cols=1, 
                            subplot_titles=("Loss (Train & Val)", "Metrics (Acc, UF1, UAR)"),
        ...
        fig.write_html(os.path.join(current_dir, f'interactive_metrics_c11_fold{fold+1}.html'))
    ...
    loss_summary_fig.write_html(os.path.join(current_dir, 'all_folds_loss_comparison_c11.html'))
    print("Saved loss summary comparison to: all_folds_loss_comparison_c11.html")
    ...
    metrics_summary_fig.write_html(os.path.join(current_dir, 'all_folds_metrics_comparison_c11.html'))
    print("Saved metrics summary comparison to: all_folds_metrics_comparison_c11.html")
