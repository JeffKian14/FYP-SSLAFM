...
from Folder_access_SAMM import SAMMDataset
...
def train_finetuning():
    ...
    dataset_path = os.path.join(project_root, "Dataset", "models_Preprocess", "SAMM_preprocessed")
    csv_path = os.path.join(project_root, "Dataset", "SAMM_Cleaned_Ready.csv")  
    ...
    train_dataset_full = SAMMDataset(image_root=dataset_path, csv_path=csv_path, transform=train_transform)
    val_dataset_full = SAMMDataset(image_root=dataset_path, csv_path=csv_path, transform=val_transform)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_dataset_full)):
        ...
        for epoch in range(EPOCHS):
            ...
            if UF1 > best_val_uf1:
                ...
                torch.save(model_s2.state_dict(), os.path.join(current_dir, f"stage3_finetuned_backbone_S1_fold{fold+1}.pth"))
                torch.save(classifier.state_dict(), os.path.join(current_dir, f"stage3_best_classifier_S1_fold{fold+1}.pth"))
            else:
                patience_counter += 1
        ...
        fig.write_html(os.path.join(current_dir, f'interactive_metrics_S1_fold{fold+1}.html'))
    ...
    loss_summary_fig.write_html(os.path.join(current_dir, 'all_folds_loss_comparison_S1.html'))
    print("Saved loss summary comparison to: all_folds_loss_comparison_S1.html")
    ...
    metrics_summary_fig.write_html(os.path.join(current_dir, 'all_folds_metrics_comparison_S1.html'))
    print("Saved metrics summary comparison to: all_folds_metrics_comparison_S1.html")
