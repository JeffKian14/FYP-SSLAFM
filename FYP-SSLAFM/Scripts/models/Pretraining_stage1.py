...
class MicroExpressionDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        """
        Args:
            root_dir (string): Directory with all the preprocessed images.
        ...
        # 1. SCAN THE FOLDERS
        # We need to group images by their video folder (e.g., sub01/EP02...)
        # Logic: Walk through, find folders that contain images, treating them as sequences.
        for root, dirs, files in os.walk(root_dir):
            # Sort files to ensure temporal order (img1, img2, img3...)
            images = sorted([f for f in files if f.endswith(('.jpg'))])
            
            if len(images) > 0:
                # We found a folder with images!
                # 2. SELECT FRAMES (The "Middle Frame" Logic)
                # Onset = First Frame
                # Offset = Last Frame
